...
#' @param weis `character(1)` - name of a certain wei_*_c2c column, added by cat2cat function. Default `wei_freq_c2c`.
...
plot_c2c <- function(data, weis = "wei_freq_c2c", type = c("both", "hist", "bar")) {
  ...
  stopifnot("There are no cat2cat additional wei_* columns/variables" = weis %in% colnames(data))
