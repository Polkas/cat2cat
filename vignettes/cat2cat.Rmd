...
These graphs present how `cat2cat::cat2cat` functions works, in this case for a panel dataset without the unique identifiers and only two periods.
...
When only three periods have to be mapped, the middle one
might be used as the base one. However, when we have to map many time points,
cat2cat has to be used recursively when pruning methods might be
needed to limit the growing number of replications.
...
freq_df <- occup_simple_1$old[, c("g_new_c2c", "wei_freq_c2c")] %>%
  group_by(g_new_c2c) %>%
  summarise(counts = round(sum(wei_freq_c2c)))

occup_simple_2 <- cat2cat(
  data = list(old = occup_2006, 
              new = occup_simple_1$old, 
              cat_var = "g_new_c2c", 
              time_var = "year", 
              freq_df = freq_df),
  mappings = list(trans = trans, direction = "backward")
...
occup_2006_new <- occup_simple_2$old
occup_2008_new <- occup_simple_1$old # or occup_simple_2$new
occup_2010_new <- occup_simple_1$new
# possibly have to add a few more cols manually, e.g. when applied ml models
...
counts_new <- final_data %>% 
  group_by(year) %>% 
  summarise(n = as.integer(round(sum(wei_freq_c2c))), .groups = 'drop')
...
# the counts could be any of wei_* or their combination
freq_df <- occup_simple_1$new[, c("g_new_c2c", "wei_freq_c2c")] %>%
  group_by(g_new_c2c) %>%
  summarise(counts = round(sum(wei_freq_c2c)))

occup_simple_2 <- cat2cat(
  data = list(old = occup_simple_1$new, 
              new = occup_2012,
              cat_var = "g_new_c2c", 
              time_var = "year", 
              freq_df = freq_df),
...
occup_2008_new <- occup_simple_1$old
occup_2010_new <- occup_simple_1$new # or occup_simple_2$old
occup_2012_new <- occup_simple_2$new
