...
More precisely for each observation we look across a transition table to check how the original category could be mapped to the opposite period one. Then using simple frequencies or statistical methods to approximate weights (probabilities) of being assigned to each of them.
...
Graph of 30 first observations from the `trans` mapping table.
This graph is specific as the newer codes are much more detailed than older ones. 
That is why almost each old 4-digit code has a few new 6-digit code siblings.

```{r, fig.height=4, fig.width=5, echo=FALSE}
oldMargins <- par("mar")
par(mar = c(1,1,1,1))
gg <- igraph::graph_from_data_frame(trans[1:30, ])
plot(gg, edge.arrow.size = .5, vertex.color = "gold", vertex.size = 15,
...
If the panel datset is balanced so contains subjects id's then we could match some of the categories directly.
Unfortunately we have to assume that a subject could not change the category level in time.
