...
1. Type of the data - panel dataset with unique identifiers vs panel dataset without unique identifiers and aggregate data vs non-aggragate data.
...
occup_2008_new <- occup_back_2008_2010$old # or occup_back_2006_2008$new
occup_2010_new <- occup_back_2008_2010$new
occup_2012_new <- dummy_c2c_cols(occup_2012, cat_var = "code", ml = c("wei_knn_c2c"))
...
ff <- final_data_back_ml %>% 
      split(.$year) %>% 
      lapply(function(x) x %>% cross_c2c() %>% prune_c2c(column = "wei_cross_c2c", method = "highest1")) %>% 
      bind_rows()
