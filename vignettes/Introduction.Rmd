...
cor(occup_3$old$wei_ml_c2c, occup_3$old$wei_c2c, use = "complete.obs")
...
```{r}
get_freqs(x = occup_3$new$g_new_c2c, multipier = floor(occup_3$new$multipier * occup_3$new$wei_c2c)) %>% head()
```

or backward:

```{r}
get_freqs(x = occup_2$old$g_new_c2c, multipier = floor(occup_2$old$multipier * occup_2$old$wei_c2c))%>% head()
```
