...
cat2cat have to be used recursively where the prune methods (`prune_cat2cat`) might be needed to limit the problem of exponentially growing replications.
...
Another solution might be using the prune_cat2cat function to remove additional rows and leave only categories with highest probabilities.
...
Without ml subsection only simple frequencies are assessed. When ml model is broken then weights from simple frequencies are taken. Method knn is recommended for smaller datasets.
